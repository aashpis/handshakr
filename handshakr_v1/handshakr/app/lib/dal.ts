...
export async function getUserProfile(userId: string ) {
  try {
    const response = await axiosInstance.get(`${API.USER_PROFILE}${userId}`);
...
export async function getHandshakeHistory(data: { username: string; password: string }){
  try {
    const response = await axiosInstance.get(API.HANDSHAKE.HISTORY, data);
    return { success: true, data: response.data };
  } catch (error: any) {
...
export async function connectagreerer(handshakeId: string, userId: string){
  try {
      const response = await axiosInstance.post(API.HANDSHAKE.CONNECT_agreererER, handshakeId);
      return { success: true, data: response.data };
    } catch (error: any) {
